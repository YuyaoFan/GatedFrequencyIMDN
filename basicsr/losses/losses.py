...
class L1CosineLoss(nn.Module):
    """
    复合损失函数：L1 Loss + Cosine Similarity Loss
    L1 负责像素级的准确度，Cosine Similarity 负责特征/颜色向量方向的一致性。
    """
    def __init__(self, loss_weight=1.0, cos_weight=0.1, reduction='mean'):
    ...
    def forward(self, pred, target):
        ...
        # 2. 计算 Cosine Similarity Loss
        # 将图像张量展平为特征向量进行计算 (B, C, H, W) -> (B, C, H*W)
        # 也可以在通道维度上计算相似度
        pred_flat = pred.view(pred.size(0), pred.size(1), -1)
        target_flat = target.view(target.size(0), target.size(1), -1)
        
        # cosine_similarity 返回值在 [-1, 1] 之间，1表示完全相同
        # 我们希望 loss 越小越好，所以用 1 - similarity
        cos_sim = F.cosine_similarity(pred_flat, target_flat, dim=2)
        cos_loss = (1 - cos_sim).mean()

        return self.loss_weight * l1_loss + self.cos_weight * cos_loss